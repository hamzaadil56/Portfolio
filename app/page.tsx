...
import { ArrowRight, Github, ExternalLink, Mail, Code, Briefcase, GraduationCap, Award } from 'lucide-react';
...
export default function Home() {
  ...
  const skills = [
    "React.js", "Next.js", "Node.js", "Python", "TypeScript", "AWS", 
    "MongoDB", "PostgreSQL", "FastAPI", "OpenAI", "Redux", "Tailwind CSS"
  ];

  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-slate-100 dark:from-slate-950 dark:via-blue-950 dark:to-slate-900 relative overflow-hidden transition-colors duration-500">
      {/* Animated background elements */}
      <div className="absolute inset-0 overflow-hidden">
        <div 
          className="absolute w-96 h-96 bg-blue-500/10 dark:bg-blue-500/10 rounded-full blur-3xl hidden lg:block"
          style={{
        ...
        <div className="text-center">
          <motion.div variants={fadeInUp} className="mb-6">
            ...
            <p className="text-lg md:text-xl text-slate-600 dark:text-white/70 leading-relaxed mb-6 md:mb-8 max-w-4xl mx-auto">
              Software Engineer with over 2 years of experience building scalable web applications, 
              AI-powered solutions, and cloud-native architectures. Passionate about creating 
              production-ready solutions that make a real impact.
            </p>
          </motion.div>

          <motion.div variants={fadeInUp} className="flex flex-col sm:flex-row gap-4 mb-6 md:mb-8 justify-center">
            <Link 
              href="/projects"
      ...
      >
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-8">
          {[
            ...
            { icon: GraduationCap, label: "Education", value: "B.E." },
            { icon: Award, label: "Hackathon Awards", value: "Top 10" }
          ].map((stat, index) => (
            <motion.div
              key={stat.label}
              initial={{ opacity: 0, y: 20 }}
            ...
            >
              ...
              <h3 className="text-xl md:text-2xl font-bold text-slate-800 dark:text-white mb-1 md:mb-2 heading-fix">{stat.value}</h3>
              <p className="text-slate-600 dark:text-white/60 text-sm md:text-base">{stat.label}</p>
            </motion.div>
          ))}
        </div>
      </motion.section>
